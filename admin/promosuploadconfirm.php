<?php
        ...
        if (isset($menu) && isset($path)) {
            if (isset($_POST['menusubtmiconfirm'])) {
                ...
                if (isset($_POST['menuconfirmtitle'])) {
                    ...
                    $sql = "SELECT title FROM tbl_promos". " WHERE id =".$_SESSION['promosconfirm'];
                    $result  = mysqli_query($connection , $sql);
                ...
                if (isset($_POST['menuconfirmcaption'])) {
                    ...
                    if (strlen($_POST['menuconfirmcaption']) === 0) {
                        ...
                        $sql = "SELECT caption FROM tbl_promos". " WHERE id =".$_SESSION['promosconfirm'];
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            while($rows = mysqli_fetch_assoc($result)) {
                                $_SESSION['descriptionconfirmsuccess'] = $rows['description'];
                            }
                        } 
                    }
                ...
                if (isset($_FILES['menuconfirmimg']) && $_FILES['menuconfirmimg']['error'] === 0) {
                   ...
                   if (mysqli_num_rows($result) > 0) {
                        while ($rows = mysqli_fetch_assoc($result)) {
                            ...
                            $_SESSION['confirmpromospath'] = $rows['path'];
                        }
                   }
                }
            }
            ...
            if (isset($_SESSION['confirmfilepath']) && isset($_SESSION['titleconfirmsuccess']) &&
                ...
                $sql = "UPDATE ".$_SESSION['database_name']. " SET" .
                 " title = '$titleSuccess' , image = '$filePath' , description = '$DescriptionSuccess' ,    path = '$dirPath'".
                 " WHERE id = ".$_SESSION['promosconfirm'];
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        $sql = "SELECT * FROM ". $_SESSION['database_name']. " WHERE id = " . $_SESSION['promosconfirm'];
        //die($sql);
        $query = mysqli_query($connection , $sql);
       // sql($query);
        if (mysqli_num_rows($query) > 0) {
             ProcessMenu($_SESSION['promosconfirm'] , $_SESSION['menupath']);
