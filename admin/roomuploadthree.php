     ...
     function UploadFile($img) {
        ...
        if(isset($img)) {
            ...
            if (in_array($filetype , $allowed)) {
                ...
                if (file_exists("../img/services/overlayimages/ktv3/".$filename)) {
                    unlink("../img/services/overlayimages/ktv3/".$filename);
                    move_uploaded_file($filetmpname, "../img/services/overlayimages/ktv3/".$filename);
                    $_SESSION['confirmuploadstatus'] = true;
                    $_SESSION['dirpathconfirm'] = "img/services/overlayimages/ktv3/".$filename;
                    return $filename;
                } else {
                    move_uploaded_file($filetmpname, "../img/services/overlayimages/ktv3/".$filename);
                    $imgPath = $filename;
        ...
        if (isset($_POST['btnservicesbtn'])) {
            if (isset($_FILES['roomimg'])) {
                ...
                if (mysqli_num_rows($result) == 5) {
                    mysqli_close($connection);
                    $_SESSION['error'] = "<span><strong class=\"white-text\">The maximum image for room two is 5!</strong></span>\n";
                    header("location:ktvservices.php");
                }
            ...
            if (isset($_SESSION['confirmuploadstatus'])) {
                ...
                $sql= "INSERT INTO tbl_services_ktv_room3(path) VALUES('$path')";
