        ...
        if (isset($_POST['roomdeletesubmit'])) {
            if (isset($_POST['roomdelete'])) {
                if (is_numeric($_POST['roomdelete'])) {
                    ...
                    $sql = "SELECT * FROM tbl_roomonetc WHERE id = ". $roomdeleteid;

                    $result = mysqli_query($connection , $sql);

                    if (mysqli_num_rows($result) > 0) {
    
                        $sql = "DELETE FROM tbl_roomonetc WHERE id = ".$preventSQl;
            ...
            mysqli_close($connection);
            $_SESSION['termsandconditionerror'] = "<span><strong class=\"white-text\">Select message needs to be deleted!</strong></span>\n";
            header("location:termsandcondition.php");
            die();
        }
    } else {
