   ...
   session_start();
    ...
    if (isset($_SERVER['REQUEST_METHOD']) == "POST") {
        if (isset($_POST['btndelete'])) {
            $id = sanitizedData($_POST['packagedelete']);
        
            $sqlPreventInjection = mysqli_real_escape_string($connection , $id);

            if (is_numeric($sqlPreventInjection)) {
                $sql = "SELECT * FROM tbl_services_martinas_package1 WHERE id = ".$id;
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    unlink("../" . $path);
                    $sql = "DELETE FROM tbl_services_martinas_package1  WHERE id = " .$id;
